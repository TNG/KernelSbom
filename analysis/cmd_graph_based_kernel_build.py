...
def _copy_additional_build_files(src_tree: Path, cmd_src_tree: Path, output_tree: Path) -> None:
    patterns = [
        "Makefile",
        "Kconfig",
        "Kconfig.*",
    ...
    for pattern in patterns:
        ...
        for file_path in src_tree.rglob(pattern):
            if not file_path.is_file() or _is_path_within(file_path, output_tree):
                continue
            print(file_path)
...
if __name__ == "__main__":
    ...
    _copy_additional_build_files(src_tree, cmd_src_tree, output_tree)
